...
const { data: page } = useSanity(`*[_type == "page" && slug.current == "/"][0]{heroTitle, heroSubtitle, heroImage, heroCta{label, url}, features[]{title, description, icon}, statsHeading, stats[]{value, label, prefix, suffix}, processHeading, processSubheading, steps[]{title, description, icon}, testimonialsHeading, testimonials[]{quote, author, role, rating}, pricingHeading, pricingSubheading, plans[]{name, price, period, description, features, highlighted, ctaLabel, ctaUrl}, faqHeading, faqSubheading, faqs[]{question, answer}}`);
